let paginaAtual = 1;
        ...
        function gerarPaginas() {
            ...
            for (let pagina = 1; pagina <= 3; pagina++) {
                ...
                for (let i = 0; i < 20; i++) {
                    ...
                    for (let j = 0; j < 3; j++) {
                        combinacoes.push(gerarAleatorio());
                    }
                    
                    const numerosTexto = combinacoes.join('\n');
                    caixinha.innerText = numerosTexto; // Exibe os números verticalmente
                    ...
                    // Adiciona a validade com a data atual
                    const dataValidade = document.createElement('div');
                    dataValidade.classList.add('subtitulo');
                    dataValidade.innerText = `Validade: ${obterDataAtual()}`;
                    caixinha.appendChild(dataValidade);
        ...
        function imprimirPaginas() {
            ...
            paginas.forEach((pagina, index) => {
                pagina.style.display = index + 1 === paginaAtual ? 'block' : 'none';
            });
        }
